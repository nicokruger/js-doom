describe("Quad tree", function () {
    it("can place an object, represented by a point into the correct quad", function () {
        var q = new renderlib.quadtree.Quad(50,50);

        q.add(renderlib.quadtree.PointPlacer($V(10,10)));
        q.add(renderlib.quadtree.PointPlacer($V(60,10)));
        q.add(renderlib.quadtree.PointPlacer($V(10,60)));
        q.add(renderlib.quadtree.PointPlacer($V(60,60)));
        // the funny edge case
        q.add(renderlib.quadtree.PointPlacer($V(50,50)));
    ...
    it("can place an object, represented by a polygon into the correct quad", function () {
        ...
        q.add(renderlib.quadtree.PolyPlacer(p1));
        q.add(renderlib.quadtree.PolyPlacer(p2));
        q.add(renderlib.quadtree.PolyPlacer(p3));
        q.add(renderlib.quadtree.PolyPlacer(p4));
        ...
        q.add(renderlib.quadtree.PolyPlacer(p5));
        // test the edge case
    ...
    it("can create a recursive quadtree by combining quads", function () {
        var tl = new renderlib.quadtree.Quad(25,75); var tr = new renderlib.quadtree.Quad(75,75);
        var bl = new renderlib.quadtree.Quad(25,25); var br = new renderlib.quadtree.Quad(75,25);

        spyOn(bl, "add"); spyOn(br, "add"); spyOn(tr, "add"); spyOn(tl, "add");

        var qt = new renderlib.quadtree.QuadTree(50, 50, bl, br, tr, tl);

        qt.add(renderlib.quadtree.PointPlacer($V(10,10)));
        expect(bl.add).toHaveBeenCalled();
        qt.add(renderlib.quadtree.PointPlacer($V(60,60)));
        expect(tr.add).toHaveBeenCalled();
        qt.add(renderlib.quadtree.PointPlacer($V(60,10)));
        expect(br.add).toHaveBeenCalled();
        qt.add(renderlib.quadtree.PointPlacer($V(10,60)));
        expect(tl.add).toHaveBeenCalled();

        // The edge case
        qt.add(renderlib.quadtree.PointPlacer($V(50,50)));
        expect(bl.add).toHaveBeenCalled();
        expect(br.add).toHaveBeenCalled();
        expect(tr.add).toHaveBeenCalled();
        expect(tl.add).toHaveBeenCalled();
    ...
    it("can determine the relative positioning of a point to a square", function () {
        var a = renderlib.quadtree.square(0,0,100,100);
        var p = $V(50,50);
        ...
        a = renderlib.quadtree.square(90,90,100,100);
        f = jasmine.createSpy();
        a.bottomLeft(p.x, p.y, f);
        a.bottomRight(p.x, p.y, f);
        a.topLeft(p.x, p.y, f);
    ...
    it("can retrieve the objects in a quad for a given viewport", function() {
        var a = renderlib.quadtree.square(0,0,10,10);
        var p = $V(50,50);

        var q = new renderlib.quadtree.Quad(50,50);
        q.add(renderlib.quadtree.PointPlacer($V(10,10)));
        q.add(renderlib.quadtree.PointPlacer($V(20,20)));
        q.add(renderlib.quadtree.PointPlacer($V(100,100)));
    ...
    it("can iterate through items which are in more than one quad", function () {
        ...
        var l = [];
        q.forEach(renderlib.quadtree.square(49,49,51,51), function (x) {
          l.push(x);
        });
    ...
    it("can iterate through items which are in more than one quad", function () {
        ...
        var l = [];
        qt.forEach(renderlib.quadtree.square(49,49,51,51), function (x) {
          l.push(x);
        });
        expect(l.length).toBe(1);
    })
    ...
    it("can create a quad tree for given dimensions with all quads smaller than a specified size #2", function() {
        var qt = renderlib.quadtree.setupQuadTree(0,0,10,10,2.5,2.5);
