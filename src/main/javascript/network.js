...
Network.prototype.towersServicing = function(zone, towers) {
	...
	serviced = _.map(towers, function(tower) {
...
Network.prototype.callDistribution = function(zone, towers) {
	...
	distribution = _.map(towers, function(tower) { return connected_towers.indexOf(tower) != -1 ? 1 : 0 });
	return _.map(distribution, function (d) { return d * (zone["calls"]/c )});
}

// Calculate the spare capacity of all the towers in the network
Network.prototype.towerCapacities = function(zones, towers) {
	var that = this; // eish
	network_calls = _.map(zones, function (zone) { return that.callDistribution(zone, towers);} )
	tower_load = _.reduce(network_calls, function (a,b) { return zip(a,b,function (x,y) { return x+y; }) }, empty_array(towers.length));
	return zip(towers, tower_load, function (a,b) { return a["capacity"] - b } )
}

Network.prototype.generateCapacity = function(towers) {
	return _.reduce(towers, function(a,b) { a += b["capacity"]; return a}, 0);
}
